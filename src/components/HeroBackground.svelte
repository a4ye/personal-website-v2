<script lang="ts">
    ...
    const fragmentShaderSource = `
        ...
        void main() {
            ...
            // Mouse-driven center for liquify pass 1 (reduced sensitivity)
            vec2 center1 = vec2(0.5, 0.5) + (u_mouse - 0.5) * 0.15;
            ...
            // Liquify pass 2: mouse-driven distortion with strong chromatic aberration
            vec2 center2 = vec2(0.5, 0.5) + (u_mouse - 0.5) * 0.1;
            float freq2 = 5.0 * (1.27 + 0.1);
            float amp2 = 0.23 * mix(0.2, 0.2 / (1.27 + 0.05), 0.25) * 0.4;
            ...
            vec3 color = texture2D(u_texture, finalG).rgb;
            ...
            // Selective hue remap: push yellow/green/cyan (0.08–0.55) toward blue-purple
            // Pinks (0.0–0.08) and blues/purples (0.55+) are left mostly untouched
            float warmness = smoothstep(0.08, 0.25, h) * (1.0 - smoothstep(0.48, 0.62, h));
            h = fract(h + warmness * 0.25);

            // Vibrance: tame highly-saturated areas without graying out soft pastels
            s *= (1.0 - s * 0.45);

            // Lift lightness for an airy, bright feel
            l = min(l * 1.1, 1.0);
            ...
            if (s < 0.001) {
                ...
                graded.r = t3.x < 1.0/6.0 ? p+(q-p)*6.0*t3.x : t3.x < 0.5 ? q : t3.x < 2.0/3.0 ? p+(q-p)*(2.0/3.0-t3.x)*6.0 : p;
                graded.g = t3.y < 1.0/6.0 ? p+(q-p)*6.0*t3.y : t3.y < 0.5 ? q : t3.y < 2.0/3.0 ? p+(q-p)*(2.0/3.0-t3.y)*6.0 : p;
                graded.b = t3.z < 1.0/6.0 ? p+(q-p)*6.0*t3.z : t3.z < 0.5 ? q : t3.z < 2.0/3.0 ? p+(q-p)*(2.0/3.0-t3.z)*6.0 : p;
            }

            // Pink-lavender tint: warm the pinks, lift the blues
            graded.r = min(graded.r * 1.04, 1.0);
            graded.b = min(graded.b * 1.07, 1.0);
